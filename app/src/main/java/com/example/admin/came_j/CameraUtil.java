...
public class CameraUtil {
    ...
    public static int mCameradeID = Camera.CameraInfo.CAMERA_FACING_FRONT;
    ...
    public static void openFrontalCamera(int expectFps) {
        if (mCamera != null) {
            throw new RuntimeException("camera  already    initalized!");
        }
        ...
        if (mCamera == null) {
            mCamera = Camera.open();
            mCameradeID = Camera.CameraInfo.CAMERA_FACING_BACK;
        }
        //没有摄像头时，抛出异常
        if (mCamera == null) {
            throw new RuntimeException("没有摄像头还行拍照？");
        }
        Camera.Parameters parameters = mCamera.getParameters();
        mCameraPreviewFps = CameraUtil.chooseFixedPreviewFps(parameters, expectFps * 1000);
        parameters.setRecordingHint(true);
        mCamera.setParameters(parameters);
        setPreviewSize(mCamera, CameraUtil.DEFAULT_WIDTH, CameraUtil.DEFAULT_HEIGHT);
        setPictureSize(mCamera, CameraUtil.DEFAULT_WIDTH, CameraUtil.DEFAULT_HEIGHT);
    ...
    public static void switchCamera(int cameraID, SurfaceHolder holder) {
        ...
        releaseCamera();
        // 打开相机
        openCamera(cameraID, CameraUtil.DESIRED_PREVIEW_FPS);
        // 打开预览
    ...
    public static int calculateCameraPreviewOrientation(Activity activity) {
        Camera.CameraInfo info = new Camera.CameraInfo();
        Camera.getCameraInfo(mCameradeID, info);
